...
- **Cannon (player)** — Bottom-aligned **grid-snapped** ship (**`Player`**: discrete horizontal steps of half bar width, **difficulty-scaled per-direction cooldown**). **`RenderSystem.DrawPlayerCannon`** draws a **composed silhouette** (base, mid, thin barrel), **neon outline**, optional **silhouette glow**, **energy core** (idle pulse + brighter flash while **`MuzzleFlashFrames`** run), **gold muzzle** + aim line, and a short **recoil** (translate with shield FX) on fire. Hitbox for movement/collision remains **`Player.GetBounds()`**.
...
- **Player / shots** — Cannon drawn as stacked rects + core (see above); **not** drawn while **`IsFinalDeathAnimating`**. Bullets: **cosmetic** outer glow + bright core on the same **`GetBounds()`** AABB, **rounded nose** (`FillEllipse`), **faded rect trails** beneath; pierce rounds stay **magenta** with stronger glow/trails. **Muzzle flash** on fire.
...
Shake transform → background + grid → danger line → bars (fill + outline + hit pulse) → particles + fragments → bullet trails → bullets (glow + core + trails; **magenta pierce**) → player (**cannon + shield skipped** while **`IsFinalDeathAnimating`**) → **state-based UI** ( **`FinalDeathFlashFrames`** warm flash during last-life breakup) → game-over overlay (**FINAL RESULTS** + leaderboard) → debug (hidden while leaderboard overlay is shown).
