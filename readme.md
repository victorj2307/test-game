...
| **Graphics** | **GDI+** via `System.Drawing` | `Graphics`, `Pen`, `Brush`, `Rectangle`, `Color`, `Font`, `SmoothingMode`, etc. Gameplay rendering runs from `GameForm.OnPaint` → `RenderSystem.Draw` (not per-entity controls). World/FX draw with **`SmoothingMode.None`**; cannon, HUD, and **explosion/shield arcs** use **AntiAlias**. Particles use per-spark **`Size`** (larger cores, not more count). Animated FX (explosions, shield barrier, bomb fuse) use **mutable pens/brushes**. Stable pens/brushes/fonts use **bounded LRU caches** with packed **long/int keys**; **`RenderSystem.DisposeSharedResources`** clears caches and the backdrop bitmap when the main form closes. HUD score/best/combo strings are cached until values change. The synthwave backdrop is cached to a **`Bitmap`** and rebuilt only on resize. |
...
- **Cannon (player)** — Bottom-aligned **grid-snapped** ship (**`Player`**: discrete horizontal steps of half bar width, **difficulty-scaled per-direction cooldown**). **`RenderSystem.DrawPlayerCannon`** draws a **readable ground turret**: wide chassis (bottom ~⅔ of the AABB) with a darker under-lip, a **thicker centered barrel**, a small gold panel light on the hull, **one T-silhouette outline**, **gold muzzle** + aim line, and muzzle flash / outline pulse while **`MuzzleFlashFrames`** run; short **recoil** (cannon translate only) on fire. Hitbox for movement/collision remains **`Player.GetBounds()`**.
...
- **Life loss / game over** — If a bar reaches the floor (and dev mode is off), **`GameState.TryConsumeShield(impactX)`** runs first: with **`ShieldCharges`** active, the bar is removed, **no life is lost**, and shield feedback (sound, floor ripple at the bar, flash, light shake) plays. Otherwise **`GameState.LoseLife()`** is called. If lives remain, the playfield is cleared and **`ResetAfterLifeLost`** runs, then the game enters a paused **`IsLifeLost`** state until the player continues. **Last life:** the playfield is cleared, **`SpawnCannonDestructionFragments`** spawns a **large** breakup: many **`Fragment`** shards (grid + radial + fast spray, tunable via **`Effects.CannonDestruction*`**), a dense omnidirectional **`Particle`** burst at the hull, then **`BeginFinalDeathAnimation`** sets **`IsFinalDeathAnimating`**, a brief **full-screen flash** (**`FinalDeathFlashFrames`**), shake, and **`SimulationTimeScale`** eases from **`FinalDeathTimeScaleMin`** → **1** over **`FinalDeathTimeScaleRecoverMs`** (~**2s** wall clock by default, smoothstep) while **particles and fragments** simulate (**`deltaSeconds * SimulationTimeScale`**). **`GameState.AdvanceFrame`** still uses **wall-clock** `deltaSeconds` so HUD frame timers stay aligned with the WinForms timer. When recovery finishes, **`ApplyGameOverShakeAndClearMuzzle`** → **`SetGameOver`** (leaderboard, timer stop, etc.). **Dev mode** (`GameState.IsDevMode`): bars that reach the floor are **removed** with a short shake and **do not** cost a life (shield still consumes first if active).
- **Progressive difficulty (tuning table)** — Difficulty is driven by explicit keyframes (time in seconds + target bar speed + target spawn interval + target max bars). Table samples ease toward targets each frame (`SmoothInt`); destroys call **`SyncDifficultyAfterDestroy`** with **`force: false`** so mid-run kills do not snap difficulty. Immediate apply (`ApplyDifficultyImmediate`) is reserved for reset / dev toggle.
- **Parameter mapping (independent)** — At runtime, `DifficultySystem` samples the table by current run time and linearly interpolates between neighboring keyframes, then blends the resulting targets into `BarSpeed`, `SpawnIntervalFrames`, and `DynamicMaxBarsOnScreen`.
...
- **Spawn reachability** — Near the concurrent bar cap (pressure = count ≥ cap − 1), new spawns must land inside a player-centered band: half-width ≈ fall-time × player traverse speed × **`ReachabilitySafetyFactor`** (clamped). Earlier, wider placement is allowed, but opening a far lane opposite an existing far extreme is rejected so both screen edges cannot fill beyond one traverse budget.
...
- **HUD** — Visual HUD with **life icons** (top-left; count matches **`MaxLives`**; omitted on the life-lost overlay so hearts are not drawn twice), a boxed **SCORE** label+value (top-right), secondary **BEST** beneath it, always-visible **COMBO xN** (dim at x1, emphasized when streak **> 1**), floating **`+N`** score popups at destroy sites, and a **centered active power-up card**: large colored glyph on top, short **uppercase** label beneath (**RAPID**, **MULTI**, **PIERCE**, **SHIELD**, **SLOW**), plus a **timer bar** when the effect is timed. **Bomb** uses a playfield fuse marker instead of that timed card. When shield charges remain under another buff, a small **shield pip** appears beside lives. Attract mode shows **BEST** only. **NEW BEST!** banners sit under the power-up card.
- **Overlays** — Attract: **READY?** + **Enter or click Start · Scores for leaderboard** + control strip (Move / Fire / Pause). **Scores** opens an attract-only leaderboard browse (board only; **Esc** or **Back** to return). Pause: semi-transparent overlay with **PAUSED** and an ESC continue hint. Life lost: semi-transparent overlay with **LIFE LOST**, remaining life icons, continue hint, and dedicated life-lost SFX. Game over: **FINAL RESULTS** + **GAME OVER** + leaderboard (rows shrink to fit above the button stack) with hint **Enter: Play again · Menu: title** in the band above the buttons.
- **Status bar** — Docked bottom **`Panel` + `Label`**; play height = client height minus bar height; **`OnPaint`** clips to the play rectangle. Attract/game-over lines emphasize how to start, open Scores, or play again (not F1-first).
...
- **Fragments & particles** — Impact sparks remain in **`EntityManager.Particles`**. A shared debris system in **`EntityManager.Fragments`** adds rectangle fragments with lifetime, gravity, and fade: bullet hits spawn tiny directional chips (subtle), bomb kills spawn larger radial shards (dramatic), and **last-life cannon destruction** spawns a **large** shard burst plus a bright **particle** cloud; **`Fragment.IsHighlight`** marks one radial shard for extra pop in **`RenderSystem.DrawFragments`**.
- **Bar types** — **Normal**, **Fast** (higher per-type speed scale, shorter height band), **Tank** (lower scale, taller). Target pixel speed is continuous **`globalSpeed × typeScale`** (min clamp; no integer round), so Fast bars do not jump **1→3** px/frame when global speed steps **1→2**. Motion eases toward that target (**`Bar`** lerp).
- **Health color** — Green → gold → red from **`HealthRatio`**; flash color overrides while flashing.
- **Combo / score** — Destroys within **`ComboTimeWindowFrames`** (90) raise streak; points = **`min(streak, 4) × (2 if special else 1)`**. Normal bars award **1 / 2 / 3 / 4**; special bars (**~12%** spawn chance) award **2 / 4 / 6 / 8**. Variation is combo (and special ×2), not a random point roll.
...
- **Background** — Synthwave backdrop in **`RenderSystem`**: multi-stop vertical gradient, sunset disk with scanlines, skyline silhouettes, and a perspective grid.
- **Danger line** — Horizontal guide; pen warms when any bar’s bottom crosses it.
- **Player / shots** — Cannon drawn as hull + barrel turret (see above); **not** drawn while **`IsFinalDeathAnimating`**. Bullets: **cosmetic** outer glow + bright core on the same **`GetBounds()`** AABB, **rounded nose** (`FillEllipse`), **faded rect trails** beneath; pierce rounds stay **magenta** with stronger glow/trails. **Muzzle flash** on fire.
- **Bars** — Thin dark outline for separation from the grid.
- **Screen shake** — ~**2–2.5 px** for a short time on destroy, life lost, and game over; **bomb detonation** adds a separate **~2 px** heavy pattern for **`BombHeavyShakeFrames`** while the flash fades. **Shield block** uses a small dedicated shake pattern (**`ShieldImpactShakeFrames`**) plus **`ShakeUntilTickMs`** extension from **`TryConsumeShield`** (does not stack with bomb’s heavy shake while bomb frames run).
- **Shield (base barrier)** — While **`ShieldCharges` > 0**, **`DrawShieldPlayerFx`** draws a quiet **violet-lavender floor band** (soft rect fill + thin top edge; distinct from the cannon’s cyan/gold and from the danger line). Drawn **behind** the cannon so the turret stays focal. Pickup/block brighten the same family; flat ripples expand from **`ShieldBlockImpactX`** at the floor.
- **Hit impact flash** — Short bright cross at the contact point (**`ImpactFlashFrames`**) so bullet hits read without more particles.
- **Life-lost flash** — Short red flash fade (`LifeLostFlashFrames`) when a life is consumed.
- **Power-ups** — Destroyed bars can drop collectibles: Rapid Fire, Multi Shot, Piercing Shot, Shield, Slow Motion, Bomb Shot. Effects are applied immediately on collection. Falling drops are colored discs with a **tiny type glyph** (same symbols as the HUD) so Rapid vs Bomb stay distinct. **Slow Motion** slows bar fall by applying **`GameState.SlowMotionBarSpeedScale`** inside **`Bar`** target pixel speed (sub-1 px/frame allowed) plus a faster speed lerp while active—**`GetEffectiveBarSpeed()`** stays the table value so difficulty UI/cooldown math stay consistent. The HUD uses a **visual-first** slot: each type has a **distinct color** and **simple drawn symbol** (speed streaks, triple dots, diamond, shield polygon, clock arc, bomb+fuse) in [`RenderSystem`](Rendering/RenderSystem.cs) (`DrawPowerUpGlyph` / `GetPowerUpHudLabel`), not long technical names. Timed effects show a **progress bar** under the label when `ActivePowerUpDurationFrames` is set. **Bomb Shot** does **not** use bullets: on pickup, **`GameState`** stores a **random point** in the playfield and a **frame fuse** (~12–31 frames ≈ **200–500 ms**); [`RenderSystem.DrawBombFuseIndicator`](Rendering/RenderSystem.cs) shows a pulsing marker; when the fuse hits zero, **`CollisionSystem.ExecuteBombExplosionAt`** runs a **hybrid cull**: active bars are **sorted by danger** (lowest on screen / largest **bottom** Y first, then distance to the blast point), then **`floor(count × 0.72)`** of them are **destroyed** (capped so at least one bar can survive when **count ≥ 2**); remaining bars inside a **splash radius** take extra damage. Culled bars are **`DestroyBarAt`**’d **over several frames** in **distance order** (closest to the blast center first; **`CollisionSystem.TickPendingBombKills`**). **`ExplosionFx`** uses a **smooth-expanding** triple shockwave (**`ExpansionT`** easing, **`MaxWaveRadius`** ~168); **`DrawBombScreenFlash`** adds a brief **white/yellow** full-screen fade (`BombScreenFlashFrames`); see [Dev mode hotkeys](#dev-mode-hotkeys-debug-builds-only) for tester shortcut **3**.
...
- **Audio** — Procedural mono **44.1 kHz** sine WAV in memory (**`SoundGenerator`**), **cosine envelope**, and preloaded clip banks backed by a **`SoundPlayer`** pool (12 slots) for overlapping non-blocking **`Play()`** (no runtime `Load()` in hot paths). Dense **hit / pierce / shoot** banks are **rate-limited** (`GameConfig.Audio.*MinIntervalMs`); **bomb** has a dedicated low boom bank (`PlayBomb`) so detonations do not share the hit rate limit. Idle slots skip **`Stop()`**. **`GameAudio`** exposes shoot / hit / bomb / life lost / game over / **shield pickup** / **shield block** / **pierce hit**.
- **Leaderboard** — **`highscore.json`** via **`HighScoreStore`** stores **Top 10** entries (`Name`, `Score`, optional `DateUtc`). Names are normalized (`Trim`, case-insensitive match), and each player keeps only a single **best score** entry (no run-to-run accumulation). On the attract screen, **Scores** opens a browse overlay (`GameState.IsBrowsingLeaderboard`) that reloads the board from disk and draws the same panel without FINAL RESULTS. On game over, qualifying scores prompt for a nickname (default **PLAYER** if blank); the overlay shows a dedicated **FINAL RESULTS** panel (Score, Best Score, Max Combo) above the leaderboard using the same panel style.
...
| **Destroy** | When **`Height ≤ 0`**: bar removed, **`GameState.RegisterBarDestroyed(isSpecial)`** awards points and returns them for a floating **`+N`** popup, then **`DifficultySystem.SyncDifficultyAfterDestroy`** re-samples with smoothing (**`force: false`**). Special bars apply a **2x score multiplier** and add slightly stronger destroy feedback. |
| **Combo** | If previous destroy was within **90 frames**, **`_combo`** increments; else reset to **1**. **Points** = **`min(_combo, 4) × (2 if special else 1)`**. HUD always shows **COMBO xN**; streak **> 1** uses the emphasized style. |
| **Leaderboard** | During play, **`HighScore`** updates in memory for HUD. Attract **Scores** browse reloads **`HighScoreStore.LoadLeaderboard`** into **`GameManager`**’s in-memory list and sets **`IsBrowsingLeaderboard`**. At game over, qualification for Top-N is checked against the **in-memory** leaderboard snapshot (**`GameManager.ScoreQualifiesFromEntries`**) so the tick path does not re-read disk before the modal prompt. If the score qualifies, name entry is requested and **`HighScoreStore.TryAddScore(...)`** persists JSON using normalized-name best-score rules (case-insensitive, one row per player), then the in-memory list reloads on successful save. |
...
4. Blend clamped results into game state with `SmoothInt` (dev mode caps still apply).

Destroys use the same smoothed path (`SyncDifficultyAfterDestroy`). **`ApplyDifficultyImmediate`** (`force: true`) is only for reset / dev-mode toggle.

After interpolation, values are eased slightly frame-to-frame to keep transitions visually smooth.

Each **`Bar`** still maps global speed × type scale to a continuous target fall speed and lerps internally (`SpeedLerpFactor` in `Bar`).
...
| **Reachability** | Half-width ≈ (`ReachabilityFallDistancePx` / fall px-per-sec) × player px-per-sec × **`ReachabilitySafetyFactor`**, clamped to **`ReachabilityMinHalfWidth`..`ReachabilityMaxHalfWidth`** (and never wider than half the client). Under pressure (bars ≥ cap − **`ReachabilityPressureSlotsFromCap`**), candidates must sit in that band around the player. Outside pressure, reject a far-side spawn if the opposite far extreme is already occupied. |
...
| `UI/GameForm.cs` | **`Forms.Timer`** tick, **`Stopwatch`** delta per tick, keys, **Start / Scores / Continue / Play again / Menu / Back** button flow, status strip, clip + **`Game.Core.GameManager.Draw`**, FPS sample for debug. |
...
| `Systems/SpawnSystem.cs` | **`TrySpawn`**, overlap/spacing validation, player-reach band filtering, overlap-aware spawn Y stacking, types/heights, **`BarWidth`**. |
| `Systems/CollisionSystem.cs` | **`Resolve`**: hits, audio/particles/fragments, destroy hooks (+ score popups), bomb wave queue, and staggered bomb fragmentation. |
| `Systems/DifficultySystem.cs` | Table-driven difficulty with smoothed blends; destroy sync does not force-snap. |
...
|--------|----------------|
| **`GameForm`** | Window, keyboard set, **`Timer`** + **`Stopwatch`**, Start/Scores/Continue/Play again/Menu button flow, status text, play height, clip, invalidate. |
...
2. **`GameState.AdvanceFrame(deltaSeconds)`** — frame index and transient timer decay (**wall-clock** `deltaSeconds`; not multiplied by **`SimulationTimeScale`**).
3. **Life-lost gate** — if `IsLifeLost`, return (normal gameplay simulation is paused).
4. **Final-death gate** — if **`IsFinalDeathAnimating`**: **`TickFinalDeathAnimation()`** updates **`SimulationTimeScale`** from wall time; **`UpdateParticles`** and **`UpdateFragments`** with **`deltaSeconds * SimulationTimeScale`**; **`DifficultySystem.Tick`**; on completion **`ApplyGameOverShakeAndClearMuzzle`**. Return (no movement, firing, spawns, or bar sim).
5. **Player phase** — bottom align, movement steps, and firing cadence.
6. **Entity phase** — `simDt = deltaSeconds * SimulationTimeScale` (normally **1**); update bullets/particles/fragments/power-ups with **`simDt`**; collect pickups; tick pending bomb fuse.
7. **Bar phase** — retarget active bar speed and move bars ( **`simDt`** + optional **Slow Motion** lerp factor).
8. **Floor-hit phase (before bullet collisions)** — bars that reach floor consume shield/life or begin **final death** / life-lost.
9. **Combat phase** — `CollisionSystem.Resolve` (no delta); **`UpdateExplosions(simDt)`**; **`TickPendingBombKills`** (frame-queued bomb destroys).
10. **Spawn phase** — `SpawnSystem.TrySpawn` when not game over / not final death.
11. **Difficulty phase** — `DifficultySystem.Tick` samples table and smooths toward targets.

### Render order (`RenderSystem.Draw`)

Shake transform → background + grid → danger line → bars (fill + outline + hit pulse) → particles + fragments → bullet trails → bullets (glow + core + trails; **magenta pierce**) → **shield base barrier** (world-anchored; under cannon) → player cannon (recoil; both skipped while **`IsFinalDeathAnimating`**) → **state-based UI** ( **`FinalDeathFlashFrames`** warm flash during last-life breakup) → game-over overlay (**FINAL RESULTS** + leaderboard) → debug (hidden while leaderboard overlay is shown).
...
|--------|----------------|
| **Lifecycle / attract / new run** | **`GameManager`** (`EnterAttractMode`, `StartNewGame`, `ResetState`, `Initialize`, **`TryOpenLeaderboardBrowse`** / **`CloseLeaderboardBrowse`**). |
| **Authoritative run numbers** | **`GameState`** (score, lives, flags including `IsLifeLost` / **`IsFinalDeathAnimating`**, **`SimulationTimeScale`**, spawn countdown/interval, `BarSpeed`, combo, active power-up/timer, shake/flash, input timers). |
...
| **Power-up activation** | **`GameManager`** + **`GameState`** (pickup activates immediately). HUD **`ActivePowerUp`** is one slot at a time; **`ShieldCharges`** can remain while a timed buff occupies the card. **Shield**: one charge (**`ShieldCharges`**), HUD **SHIELD** when that is the active card, **violet-lavender floor band** behind the cannon (**`RenderSystem.DrawShieldPlayerFx`**), pickup wash + **`GameAudio.PlayShieldPickup`**, floor block via **`TryConsumeShield(impactX)`** (floor ripple at bar X, flash, **`PlayShieldBlock`**). **Bomb**: fuse + **`ExecuteBombExplosionAt`**, then staggered **`TickPendingBombKills`** + screen flash / heavy shake + **`ExplosionFx`**. |
...
| **Shared GDI cleanup** | **`GameForm`** `FormClosed` → **`GameManager.DisposeResources`** → **`RenderSystem.DisposeSharedResources`** (dispose cached pens/brushes/fonts and backdrop bitmap). |
...
| **Menu (game over)** | Click **Menu** to return to the title / READY screen (`EnterAttractMode`) without starting a run |
| **View scores (attract)** | Click **Scores**; **Esc** or **Back** closes the browse overlay |
...
- **HUD (in-play overlay)** — Top-left lives label + heart icons (plus optional shield pip), top-right score box, best score text under it, always-visible **COMBO xN** (dim at x1), floating **`+N`** at destroys, and a **centered power-up card** (large glyph, uppercase short label, optional timer bar). Empty slot shows a dim placeholder ellipse. Temporary banners like **NEW BEST!** sit under the power-up card when applicable. Attract shows **BEST** only. **Dev mode** adds a small **DEV MODE** block (lower-left of the play area, above the F1 FPS line) with live tuning readouts.
- **Active power-up card (top-center)** — Implemented in **`RenderSystem.DrawPowerUpHud`** / **`DrawActivePowerUpCard`**. Layout: **~36×36 px** colored glyph inside a white ring, **7 pt bold** label centered under the icon, **4 px** timer strip when the run state exposes a finite duration. **Type → color / symbol / label** (all `System.Drawing` primitives):
  ...
  | `Shield` | SHIELD | Lavender (`ARGB` ~175,150,245) | Filled shield polygon |
...
- **Start / Scores / Menu buttons** — Centered near the bottom of the play area. **Primary CTA is always on top** (bright cyan **Start** / **Play again** / **Back** / **Continue**), secondary below (**Scores** on attract, **Menu** on game over). Primary uses a larger cyan fill + bright border so it reads as the main action. Flat style with hover.
...
| **Fragment / particle FX** | Caps: **`MaxActiveFragments`** (180), **`MaxActiveParticles`** (160), **`MaxActiveScorePopups`** (16). Particles carry a **`Size`** (hit cores ~5 px; bomb sparks 4–6; fewer bomb sparks). Bomb **splash** skips per-bar hit sparks (filled core + rings + dedicated boom). Special destroys use **1–2 large gold shards**. |
...
- **`Effects`** — `BombRadius`/`BombSplashDamage`/`BombSoleBarDamage`/`BombSingleTargetBonusDamage` (bomb damage profile), `BombKillFraction` (fraction of bars queued for kill), `BombKillDelayStartFrames`/`BombKillStaggerFrames` (bomb stagger timing), `MaxActiveFragments` / `MaxActiveParticles` / `MaxActiveScorePopups` (FX caps), `BombExplosionParticleCount` (bomb sparks), `BombExplosionShakeMs`/`BombScreenFlashFrames`/`BombHeavyShakeFrames` (bomb impact feedback), `LifeLostShakeMs`/`FloorHitDevShakeMs`/`GameOverShakeMs`/`DestroyShakeMs` (shake durations), `SpawnRelaxAfterLifeLostFrames` (post-life-loss spawn relief), `LifeLostFlashFrames`/`NewBestFlashFrames` (overlay timing), `BombIndicatorMargin`/`BombFuseMinFrames`/`BombFuseMaxExclusiveFrames` (bomb indicator placement/fuse range), **`FinalDeathTimeScaleMin`** / **`FinalDeathTimeScaleRecoverMs`** (last-life cinematic slow-mo), **`FinalDeathFlashFrames`** / **`FinalDeathShakeMs`** (breakup feedback), **`CannonDestructionFragmentCount`** / **`CannonDestructionParticleCount`** / speed and gravity constants (**`CannonDestructionChunkSpeedMin`**/**`Max`**, **`CannonDestructionBlastSpeedMin`**/**`Max`**, **`CannonDestructionFragmentGravity`**) for the last-life cannon detonation.
...
- **`Ui`** — `WindowWidth`/`WindowHeight` (default client size), `GameTimerIntervalMs` (tick interval), `MinPlayHeight` (min drawable playfield), `MinDeltaSeconds`/`MaxDeltaSeconds` (**`MaxDeltaSeconds` ≈ 0.05s** to limit hitch catch-up), `FpsWindowMs` (FPS averaging window).
- **`Audio`** — `HitMinIntervalMs` / `PierceMinIntervalMs` / `ShootMinIntervalMs` / `BombMinIntervalMs` (SFX rate limits), `DefaultMinIntervalMs` (other banks).
...
- **Leaderboard highlight nuance** — game-over leaderboard “current run” highlight prefers matching **name + score** after a successful submit; without a highlight name it falls back to the first matching score.
- **Attract score browse** — `IsBrowsingLeaderboard` is separate from game-over `ShowLeaderboard` so FINAL RESULTS is not shown when viewing scores from READY.
...
- Attract leaderboard browse open/close gating
- Game over → Menu / `EnterAttractMode` returns to title
